  ...
  useEffect(() => {
    ...
    const fail = (reason: string, err?: unknown) => {
      if (import.meta.env.DEV) {
        console.warn(`[MagicRings] ${reason}`, err ?? '');
      }
    ...
    try {
      ...
      fail('WebGLRenderer could not be created', e);
      return;
    }
    ...
    const animate = (t: number) => {
      ...
      uniforms.uColor.value.set(p.color ?? '#ffffff');
      uniforms.uColorTwo.value.set(p.colorTwo ?? '#3b82f6');
